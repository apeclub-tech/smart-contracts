...
contract TreeNursery is Ownable, Pausable, ReentrancyGuard {
    ...
    uint256 public initialLPValue; // Initial number of LP tokens of BNB-BANANA
    uint256 public initialLPBnb; // Initial number of BNB deposited in the LP
    ...
    function setStakeToken(IERC20 _stakeToken) external onlyOwner {
        require(!nurseryStarted(), "nursery is running");
        lpToken = _stakeToken;
    }
    ...
    function getRewardPerSecond() public view returns (uint256) {
        ...
        return rewardPerSecondCalculation(lpSupply, initialLPValue, initialLPBnb, rewardMultiplier, rewardMultiplierDivision, baseRate);
    }
    ...
    function nurseryStarted() public view returns (bool) {
    ...
    function setInitialLPBnb(uint256 _initialLPBnb) external onlyOwner {
        initialLPBnb = _initialLPBnb;
    }
    ...
    function rewardPerSecondCalculation(uint256 _lpSupply, uint256 _initialLPValue, uint256 _initialLPBnb, uint256 _rewardMultiplier, uint256 _rewardMultiplierDivision, uint256 _baseRate) public pure returns (uint256) {
        if(_lpSupply == 0){
            return 0;
        }
        
        if(_initialLPValue == 0 || _initialLPBnb == 0 || _rewardMultiplier == 0 || _rewardMultiplierDivision == 0) {
            return _baseRate;
        }
        return _baseRate + _rewardMultiplier.mul(_lpSupply).mul(_initialLPBnb).div(_initialLPValue.mul(_rewardMultiplierDivision).mul(86400));
    ...
    function deposit(uint256 _amount) public whenNotPaused nonReentrant {
        require(nurseryStarted(), "you can't deposit yet");
